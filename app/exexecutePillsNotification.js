...
module.exports = async function () {
    ...
    function getUserNotifyPills(date_format, time_format) {

        console.log("User pills");

        return new Promise((resolve,reject)=>{

            var db = require('./config/config.js').db;

            var get_pills_info = 'select t1.id,t1.sNotification_msg,REPLACE(REPLACE(REPLACE(REPLACE(t1.sNotification_title,"<Name>",t3.sName),"<noofpills>",t1.inoofpills),"<DrugName>",t2.sDrugName),"<Time>",t1.tTime) as "notification", \
            t3.sName,t1.inoofpills,t1.tTime,t2.sDrugName,t2.iUserid,t1.inotifysent \
            from tblUserPilldatesInfo t1 \
            left join tbUserPillsInfo t2 on t2.id=t1.iPillid and t2.isActive=1 \
            left join tblUserMaster t3 on t3.id=t2.iUserid and t3.sActive=1 \
            ...
            db.query(get_pills_info, [date_format, time_format, time_format], async (err, result) => {
                ...
                if (result.length != 0) {
                    ...
                    var deactive_sent_notification = "update tblUserPilldatesInfo set inotifysent=1,dModified_at=? \
                where id IN (?)";
   ...
   await getUserNotifyPills(date_format,time_format);

   //  await getUserNotifyPills("2021-04-20", "17:55:00");
